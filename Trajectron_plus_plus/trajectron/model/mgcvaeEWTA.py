...
from Trajectron_plus_plus.trajectron.model.components.map_encoder import CNNMapEncoder
import utilities
...
class MultimodalGenerativeCVAEEWTA(MultimodalGenerativeCVAE):
    ...
    def obtain_encoded_tensors(self, mode, inputs, inputs_st, labels, labels_st,
        ...
        x_concat_list = list()
        if self.hyperparams['edge_encoding']:
            x_concat_list.append(total_edge_influence)#[32]
        x_concat_list.append(node_history_encoded) #[32]
        if self.hyperparams['use_map_encoding'] and self.node_type in self.hyperparams['map_encoder']:
    ...
    def p_y_xz_split(self, x, n_s_t0, prediction_horizon):
        ph = prediction_horizon  # 12
        cell = self.node_modules[self.node_type + '/decoder/rnn_cell']
        initial_h_model = self.node_modules[self.node_type + '/decoder/initial_h']
        initial_state = initial_h_model(x)
        ...
        input_ = torch.cat([x, a_0.repeat(1, 20)], dim=1)
        features = torch.cat([input_, state], dim=1) #[104,128]
        features_history= x[:,32:]
        features_social=x[:,:32]
        features_a_0=a_0
        features_state=state
        for j in range(ph):
            h_state = cell(input_, state)
            mu_t = self.project_to_GMM_params(h_state)
            mus.append(mu_t.reshape(-1, 20, 2))
        ...
        mus = torch.stack(mus, dim=2)
        y = self.dynamic.integrate_samples(mus, x)
        return y, features_history,features_social,features_a_0,features_state

    def p_y_xz_with_features(self, x_, n_s_t0, features,prediction_horizon):
        ph = prediction_horizon  # 12
        cell = self.node_modules[self.node_type + '/decoder/rnn_cell']
        initial_h_model = self.node_modules[self.node_type + '/decoder/initial_h']
        initial_state = initial_h_model(x_)
        mus = []
        a_0=features[:,64:66]
        input_=features[:,:104]
        state=features[:,104:232]
        x=features[:,:64]
        # a_0 = self.node_modules[self.node_type + '/decoder/state_action'](n_s_t0)
        # state = initial_state
        # input_ = torch.cat([x, a_0.repeat(1, 20)], dim=1)
    ...
    def decoder_split(self,x, n_s_t0, prediction_horizon):
        y, features_history,features_social,features_a_0,features_state = self.p_y_xz_split(x, n_s_t0, prediction_horizon)
        return y,  features_history,features_social,features_a_0,features_state
    ...
    def train_loss_cluster(self,
        ...
        if contrastive:
            for n, (tra2cluster, prototypes, density) in enumerate(
                ...
                cluster_mask_positives = (
                            torch.abs(pos_proto_id.unsqueeze(-1).sub(pos_proto_id.unsqueeze(-1).T)) < 0.5).float().to(
                    features.device)
                cluster_mask_negatives = (
                            torch.abs(pos_proto_id.unsqueeze(-1).sub(pos_proto_id.unsqueeze(-1).T)) > 0.5).float().to(
                    features.device)

                device = features.device
                batch_size = features.shape[0]
                score = score.contiguous().view(-1, 1)
                score_mask_positives = (torch.abs(score.sub(score.T)) < 0.1).float().to(device)
                score_mask_negatives = (torch.abs(score.sub(score.T)) > 2.0).float().to(device)

                mask_positives=cluster_mask_positives*score_mask_positives
                mask_negatives=cluster_mask_negatives*score_mask_negatives
                ...
                contrast_loss_ = - (temp / base_temperature) * mean_log_prob_pos
                contrast_loss_ = contrast_loss_.view(1, batch_size).mean()
                contrast_loss += contrast_loss_

            contrast_loss /= len(cluster_result['tra2cluster'])

            final_loss = loss + factor_con * contrast_loss
            if self.log_writer is not None:
                self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'contrastive_loss'),
                                           contrast_loss, self.curr_iter)
                # self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'positives'),
                #                            positive, self.curr_iter)
                # self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'negatives'),
                #                            negative, self.curr_iter)
    ...
    def predict_by_feature(self,
        ...
        x, n_s_t0 = self.obtain_encoded_tensors(mode=mode,
                                                ...
                                                robot=robot,
                                                map=map)
        y, features = self.p_y_xz_with_features(x, n_s_t0,features,prediction_horizon)
        features_original=features[:]
        features_original=F.normalize(features_original,dim=1)
        cluster_features=features[:]
        if return_feat:
            features = F.normalize(self.node_modules[self.node_type + '/con_head'](features), dim=1)
        # cluster_features=F.normalize(self.node_modules[self.node_type + '/con_head'](features), dim=1)
        if return_original:
    ...
    def predict_split(self,
        ...
        x, n_s_t0 = self.obtain_encoded_tensors(mode=mode,
                                                ...
                                                robot=robot,
                                                map=map)
        y,  features_history,features_social,features_a_0,features_state = self.decoder_split(x, n_s_t0, prediction_horizon)
        features_history=F.normalize(features_history,dim=1)
        features_social = F.normalize(features_social, dim=1)
        features_a_0 = F.normalize(features_a_0, dim=1)
        features_state = F.normalize(features_state, dim=1)
    ...
    def pcl_contrast_loss_instance_future_with_score(self,
                   inputs,
                   inputs_st,
                   first_history_indices,
                   labels,
        ...
        if cluster_result is not None:
            ...
            for n,(tra2cluster,prototypes,density) in enumerate(zip(cluster_result['tra2cluster'],cluster_result['centroids'],cluster_result['density'])):
                pos_proto_id=tra2cluster[index]
                pos_prototypes=prototypes[pos_proto_id][:,:232]

                all_proto_id=[i for i in range(tra2cluster.max())]
                neg_proto_id=set(all_proto_id)-set(pos_proto_id.tolist())
                neg_proto_id=sample(neg_proto_id,len(neg_proto_id))
                neg_prototypes=prototypes[neg_proto_id][:,:232]

                instance_mask_positives=(torch.abs(pos_proto_id.unsqueeze(-1).sub(pos_proto_id.unsqueeze(-1).T))<0.5).float().to(features.device)
                instance_mask_negatives=(torch.abs(pos_proto_id.unsqueeze(-1).sub(pos_proto_id.unsqueeze(-1).T))>0.5).float().to(features.device)
                ...
                log_prob = logits - torch.log(exp_logits.sum(1,keepdim=True) + 1e-20)
                mean_log_prob_pos=(instance_mask_positives * log_prob).sum(1) / (instance_mask_positives.sum(1) + 1e-20)
...
###################################contrast loss #############################################
                ...
                instance_mask_positives_cluster=(torch.abs(pos_proto_id.unsqueeze(-1).sub(pos_proto_id.unsqueeze(-1).T))<0.5).float().to(features.device)

                instance_mask_positives_scores=(torch.abs(scores.sub(scores.T)) < 2.0).float().to(instance_mask_positives_cluster.device)
                instance_mask_negatives_scores=(torch.abs(scores.sub(scores.T)) >4.0).float().to(instance_mask_positives_cluster.device)
                instance_mask_positives_contrast=instance_mask_positives_cluster*instance_mask_positives_scores
                instance_mask_negatives_contrast=instance_mask_negatives_scores
                mask_neutral=instance_mask_positives_contrast+instance_mask_negatives_contrast
                ...
                batch_size=instance_mask_positives_contrast.shape[0]
                self_mask=torch.scatter(
                    torch.ones_like(instance_mask_positives_contrast),1,
                    torch.arange(batch_size).view(-1,1).to(instance_mask_positives_contrast.device),0
                ) * mask_neutral
                instance_mask_positives_contrast=instance_mask_positives_contrast * self_mask
                exp_logits = torch.exp(logits) * self_mask
                log_prob = logits - torch.log(exp_logits.sum(1,keepdim=True) + 1e-20)
                mean_log_prob_pos=(instance_mask_positives_contrast * log_prob).sum(1) / (instance_mask_positives_contrast.sum(1) + 1e-20)

                contrast_loss_ = - mean_log_prob_pos
                contrast_loss += contrast_loss_.view(1, batch_size).mean()
...
#################################################################################
            ...
            contrast_loss  /= len(proto_labels)
        ...
        final_loss = loss + factor_con * con_loss + factor_con*instance_loss + factor_con/5* contrast_loss
        if self.log_writer is not None:
        ...
        if self.log_writer is not None:
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'contrastive_score_loss'),
                                       contrast_loss, self.curr_iter)
    ...
    def pcl_contrast_loss_instance_future_single(self,
                                          inputs,
                                          inputs_st,
                                          first_history_indices,
                                          labels,
        ...
        xy_centroids = cluster_result['centroids'][-1]

    def pcl_contrast_loss_with_center_loss(self,
                                         inputs,
                                         inputs_st,
                                         first_history_indices,
                                         labels,
        ...
        if cluster_result is not None:
            ...
            instance_loss = 0
            instance_loss_f = 0
            for n, (tra2cluster, prototypes, density,c_loss) in enumerate(
                    zip(cluster_result['tra2cluster'], cluster_result['centroids'], cluster_result['density'],center_loss)):

                pos_proto_id = tra2cluster[index]
                c_loss_batch=c_loss[pos_proto_id]
                mask = predict_loss - c_loss_batch > thres
                ...
                self_idx=self_mask.logical_not()
                ...
                mean_log_prob_pos_f = (self_idx * log_prob).sum(1) / (
                        self_idx.sum(1) + 1e-20)
            ...
            con_loss_neg=0
            for n, (labels_proto, logits_proto) in enumerate(zip(proto_labels, proto_logits)):
                con_loss_ = criterion(logits_proto, labels_proto)
                mask_negative=mask*-1 + mask.logical_not()*1
                mask_p=mask.logical_not()
                mask_n=mask*-1
                con_loss_n_=con_loss_*mask_n
                con_loss_p_ = con_loss_ * mask_p
                con_loss_ = con_loss_ * mask_negative
                con_loss += torch.mean(con_loss_)
                con_loss_neg += torch.mean(con_loss_n_)
                con_loss_pos += torch.mean(con_loss_p_)
            con_loss /= len(proto_labels)
            con_loss_neg /= len(proto_labels)
            con_loss_pos /= len(proto_labels)

        pos_num=torch.sum(mask)
        neg_num=torch.sum(mask.logical_not())

        # final_loss = loss  + factor_con * instance_loss
        final_loss = loss + factor_con * con_loss + factor_con * instance_loss+factor_con*instance_loss_f
        # final_loss = loss + factor_con * con_loss
        if self.log_writer is not None:
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'contrastive_proto_loss'),
                                       con_loss, self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'contrastive_proto_loss_pos'),
                                       con_loss_pos, self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'contrastive_proto_loss_neg'),
                                       con_loss_neg, self.curr_iter)
        ...
        if self.log_writer is not None:
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'ewta_loss'),
                                       loss, self.curr_iter)
        if self.log_writer is not None:
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'pos_num'),
                                       pos_num, self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'neg_num'),
                                       neg_num, self.curr_iter)
    ...
    def pcl_contrast_loss_reweight_by_loss(self,
                                           inputs,
                                           inputs_st,
                                           first_history_indices,
                                           labels,
        ...
        if cluster_result is not None:
            ...
            for n, (tra2cluster, prototypes, density,
                    ...
                    zip(cluster_result['tra2cluster'], cluster_result['centroids'], cluster_result['density'],
                        cluster_result_with_future['tra2cluster'], cluster_result_with_future['centroids'], cluster_result_with_future['density'])):
            ...
            for n, (labels_proto, logits_proto, labels_proto_f, logits_proto_f) in enumerate(
                ...
                con_loss_ = criterion(logits_proto, labels_proto) * mask
                con_loss_f_ = criterion(logits_proto_f, labels_proto_f) * mask.logical_not()
                con_loss = con_loss + torch.mean(con_loss_) + torch.mean(con_loss_f_)
    ...
    def pcl_contrast_loss_reweight_by_loss_justlongtail(self,
        ...
        if cluster_result is not None:
            ...
            for n, (tra2cluster, prototypes, density,
                ...
                exp_logits = torch.exp(logits) * self_mask
                exp_logits_f = torch.exp(logits) * self_mask



                log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True) + 1e-20)
                log_prob_f = logits - torch.log(exp_logits_f.sum(1, keepdim=True) + 1e-20)

                mean_log_prob_pos = (instance_mask_positives * log_prob).sum(1) / (
                        instance_mask_positives.sum(1) + 1e-20)
                mean_log_prob_pos_f = (instance_mask_positives_f * log_prob_f).sum(1) / (
                        instance_mask_positives_f.sum(1) + 1e-20)
    ...
    def pcl_contrast_loss_instance_future_nopositivesample(self,
                   inputs,
                   inputs_st,
                   first_history_indices,
                   labels,
        ...
        if cluster_result is not None:
            ...
            for n,(tra2cluster,prototypes,density) in enumerate(zip(cluster_result['tra2cluster'],cluster_result['centroids'],cluster_result['density'])):
                ...
                instance_mask_positives=instance_mask_positives * self_mask
                exp_logits = torch.exp(logits) * self_mask*instance_mask_negatives
                log_prob = logits - torch.log(exp_logits.sum(1,keepdim=True) + 1e-20)
                mean_log_prob_pos=(self_mask * log_prob).sum(1) / (self_mask.sum(1) + 1e-20)
            ...
            instance_loss /= len(cluster_result['tra2cluster'])
    ...
    def pcl_contrast_loss_instance_future(self,
                   inputs,
                   inputs_st,
                   first_history_indices,
                   labels,
    ...
    def pcl_contrast_loss_instance_future_linear_filter(self,
                   inputs,
                   inputs_st,
                   first_history_indices,
                   labels,
        ...
        if cluster_result is not None:
            ...
            instance_loss=0
            for n,(tra2cluster,prototypes,density) in enumerate(zip(cluster_result['tra2cluster'],cluster_result['centroids'],cluster_result['density'])):
                pos_proto_id=tra2cluster[index_mask]
                pos_prototypes=prototypes[pos_proto_id][:,:232]
    ...
    def pcl_contrast_loss_instance(self,
                   inputs,
                   inputs_st,
                   first_history_indices,
                   labels,
    ...
    def pcl_contrast_loss_instance_LDAM(self,
                   inputs,
                   inputs_st,
                   first_history_indices,
                   labels,
        ...
        if cluster_result is not None:
            ...
            con_loss=0
            for n,(labels_proto,logits_proto) in enumerate(zip(proto_labels,proto_logits)):
                con_loss += criterion(logits_proto,labels_proto,cls_num_list[n])
    ...
    def pcl_contrast_loss_instance_future_twoencoders(self,
                   inputs,
                   inputs_st,
                   first_history_indices,
                   labels,
        ...
        assert other_feats is not None
        [encoder_k_feats,original_feats,original_encoder_k_feats]=other_feats
        mode = ModeKeys.PREDICT
        x, n_s_t0 = self.obtain_encoded_tensors(mode=mode,
                                                inputs=inputs,
                                                inputs_st=inputs_st,
        ...
        criterion=nn.CrossEntropyLoss()
        ############################
        q_k_contrast_pos= torch.einsum('nc,nc->n', [features, encoder_k_feats]).unsqueeze(-1)
        q_k_contrast_neg= torch.einsum('nc,ck->nk', [features, encoder_k_feats.t()])
        q_k_logit=torch.cat([q_k_contrast_pos,q_k_contrast_neg],dim=1)
        q_k_logit /= temp
        q_k_labels = torch.zeros(q_k_logit.shape[0], dtype=torch.long).cuda()
        q_k_contrast_loss=criterion(q_k_logit,q_k_labels)

        q_originalq_contrast_pos= torch.einsum('nc,nc->n', [features, original_feats]).unsqueeze(-1)
        q_originalq_contrast_neg= torch.einsum('nc,ck->nk', [features, original_feats.t()])
        q_originalq_logit=torch.cat([q_originalq_contrast_pos,q_originalq_contrast_neg],dim=1)
        q_originalq_logit /= temp
        q_originalq_labels = torch.zeros(q_originalq_logit.shape[0], dtype=torch.long).cuda()
        q_originalq_contrast_loss=criterion(q_originalq_logit,q_originalq_labels)

        q_originalk_contrast_pos= torch.einsum('nc,nc->n', [features, original_encoder_k_feats]).unsqueeze(-1)
        q_originalk_contrast_neg= torch.einsum('nc,ck->nk', [features, original_encoder_k_feats.t()])
        q_originalk_logit=torch.cat([q_originalk_contrast_pos,q_originalk_contrast_neg],dim=1)
        q_originalk_logit /= temp
        q_originalk_labels = torch.zeros(q_originalk_logit.shape[0], dtype=torch.long).cuda()
        q_originalk_contrast_loss=criterion(q_originalk_logit,q_originalk_labels)
        ...
        if cluster_result is not None:
            ...
            for n,(tra2cluster,prototypes,density) in enumerate(zip(cluster_result['tra2cluster'],cluster_result['centroids'],cluster_result['density'])):
                ...
                instance_mask_positives=(torch.abs(index.unsqueeze(-1).sub(index.unsqueeze(-1).T))<0.5).float().to(features.device)
                instance_mask_negatives=(torch.abs(index.unsqueeze(-1).sub(index.unsqueeze(-1).T))>0.5).float().to(features.device)
        ...
        final_loss = loss + factor_con * con_loss + factor_con*instance_loss + factor_con*(q_k_contrast_loss,
                                                                                          q_originalq_contrast_loss,
                                                                                           q_originalk_contrast_loss)
        if self.log_writer is not None:
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'contrastive_proto_loss'),
                                       con_loss, self.curr_iter)
    ...
    def pcl_contrast_loss_instance_with_score(self,
                   inputs,
                   inputs_st,
                   first_history_indices,
                   labels,
        ...
        contrast_loss, positive, negative = contrastive_three_modes_loss(features, score, temp=temp, change=False)


        final_loss = loss + factor_con * con_loss + factor_con*instance_loss+factor_con*contrast_loss
        if self.log_writer is not None:
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'contrastive_proto_loss'),
                                       con_loss, self.curr_iter)
...
class model_encdec(nn.Module):
    def __init__(self, hyperparams, pretrained_model=None):
        ...
        self.past_len = hyperparams['rec_past_len']  ######todo:no shorter history
        ...
        # self.temporal_decoder_x = nn.GRUCell(self.dim_embedding_key * 2,  16)
        # self.spatial_decoder_x = nn.Conv1d(16, 2, 3, stride=1, padding=1)
    ...
    def decode_state_into_intention(self, past_feature,norm_fut_state):
        # state concatenation and decoding
        input_fut = torch.cat((past_feature, norm_fut_state), 1)
    ...
    def forward(self, past_feature,future):
        norm_fut_state = self.get_state_encoding(future)
        prediction, reconstruction = self.decode_state_into_intention(past_feature,
                                                                      norm_fut_state)
        return prediction, reconstruction
...
class st_encoder(nn.Module):
    def __init__(self):
        ...
        self.temporal_encoder = nn.GRU(channel_out, self.dim_embedding_key, 1, batch_first=True)   ####todo:bidirection
