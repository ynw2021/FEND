...
class MultimodalGenerativeCVAE(object):
    ...
    def encode_edge(self,
        ...
        for i, neighbor_states in enumerate(neighbors):  # Get neighbors for timestep in batch
            if len(neighbor_states) == 0:  # There are no neighbors for edge type # TODO necessary?
                ...
                edge_states_list.append(torch.zeros((1, max_hl + 1, neighbor_state_length), device=node_history.device))
            else:
                edge_states_list.append(torch.stack(neighbor_states, dim=0).to(node_history.device))
    ...
    def eval_loss(self,
        ...
        y, features = self.p_y_xz(x, n_s_t0,
                                prediction_horizon)
