...
class ModelRegistrar(nn.Module):
    ...
    def save_models(self, curr_iter,momentum=False):
        ...
        if not momentum:
            ...
            save_path = os.path.join(self.model_dir,
                                     'model_registrar_moment-%d.pt' % curr_iter)

            torch.save(self.model_dict, save_path)

    def load_models(self, iter_num,momentum=False):
        self.model_dict.clear()

        if momentum:
            save_path = os.path.join(self.model_dir,
                                     'model_registrar_moment-%d.pt' % iter_num)
        else:
            save_path = os.path.join(self.model_dir,
