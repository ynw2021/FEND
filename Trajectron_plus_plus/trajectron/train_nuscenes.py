...
from argument_parser_baseline import args
...
def main():
    ...
    args.conf='Trajectron_plus_plus/experiments/nuScenes/models/'+'int_ee_me/config.json'
    if not os.path.exists(args.conf):
        print('Config json not found!')
    with open(args.conf, 'r', encoding='utf-8') as conf_json:
        hyperparams = json.load(conf_json)

    # kalman_score_path='../../kalman/univ_PEDESTRIAN_train.pkl'
    # with open(kalman_score_path, 'rb') as f:
    #     scores = dill.load(f, encoding='latin1')
    ...
    for epoch in range(1, args.train_epochs + 1):
        ...
        for node_type, data_loader in train_data_loader.items():
            ...
            for (batch,index,original_batch) in pbar:
                trajectron.set_curr_iter(curr_iter)
                trajectron.step_annealers(node_type)
                optimizer[node_type].zero_grad()
                train_loss = trajectron.train_loss(batch, node_type,loss_type=loss_type,score=None,contrastive=False)
