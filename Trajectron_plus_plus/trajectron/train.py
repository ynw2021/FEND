...
def main():
    ...
    for epoch in range(1, args.train_epochs + 1):
        # ewta_k=20-int(epoch/25)
        # if epoch in [100,200,300,400]:
        #     ewta_k=int(np.max([ewta_k-5,1]))
        if epoch in [100,200,300,400]:
            ewta_k=int(ewta_k/2)
        ...
        for node_type, data_loader in train_data_loader.items():
            ...
            for (batch,score,index) in pbar:
                trajectron.set_curr_iter(curr_iter)
                trajectron.step_annealers(node_type)
                optimizer[node_type].zero_grad()
                train_loss = trajectron.train_loss(batch, node_type,loss_type=loss_type,score=score,contrastive=False)
